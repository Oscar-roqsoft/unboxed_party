    ...
    <div>
        ...
        <v-container>
            <div class=" md:tw-max-w-[1000px] mx-auto">
                ...
                <v-btn color="blue-darken-4" @click.prevent="navigateTo('/admin_slym/events')" class="rounded-xl">
        ...
        <v-container>
            ...
            <v-card class="pa-4" style="background: #22222261;">
                <v-form>
                    <v-text-field class="mb-2 rounded-xl" v-model="name" label="Name" persistent-hint
                        variant="solo"></v-text-field>

                    <v-text-field class="mb-2 rounded-xl" v-model="caption" label="caption" persistent-hint
                        variant="solo"></v-text-field>

                    <v-text-field class="mb-2 rounded-xl" v-model="venue" label="venue" persistent-hint
                        variant="solo"></v-text-field>

                    <v-text-field class="mb-2 rounded-xl" v-model="time" label="time" persistent-hint
                        variant="solo"></v-text-field>

                    <v-text-field class="mb-2 rounded-xl" v-model="date" label="date" persistent-hint
                        variant="solo"></v-text-field>

                    <v-text-field class="mb-2 rounded-xl" v-model="video" label="Video link" persistent-hint
                        variant="solo"></v-text-field>

                    <v-textarea class="mb-2 rounded-xl" v-model="desc" label="description" persistent-hint
                        density="compact" variant="solo"></v-textarea>


                    <v-card-text>
                        <v-file-input accept="image/*" label="Event Image" v-model="image"
                            @input="handleImageUpload($event)" prepend-icon="mdi-image" />
                    ...
                    <v-col>
                        <v-col>
                            ...
                            <v-checkbox v-model="isActiveChecked" label="isActive" color="primary" class="mx-4">

                                <div class="">{{ isActiveChecked }}</div>
                            </v-checkbox>


                            <v-checkbox v-model="isOnSaleChecked" label="is_On_Sale" color="primary" class="mx-4">

                                <div class="">{{ isOnSaleChecked }}</div>
                            </v-checkbox>
                        </v-col>

                        <h3 class="text-h6 tw-mb-6 tw-text-center tw-font-bold"
                            style="font-family: 'Permanent Marker', cursive !important;"> Create ticket price lists</h3>

                        <v-col>
                            <v-row class="px-3">

                                <v-text-field class="mb-2 rounded-xl" v-model="ticketName" label="ticket name"
                                    persistent-hint variant="solo"></v-text-field>

                                <v-text-field class="mb-2 tw-ml-1 rounded-xl" v-model="ticketValue" label="ticket value"
                                    persistent-hint variant="solo"></v-text-field>
                        ...
                        <v-list class="pa-2 roounded tw-min-h-20">
                            <v-list-item-title class="border-b" color="primary ">list of added ticket price
                            ...
                            <div v-for="(i, index) in ticketList" :key="i.name">
                                <v-col>
                                    <v-row class="border-b tw-py-2">
                                        ...
                                        <a href="#" @click.prevent="deleteItem(i.name)"
                    ...
                    <v-btn :loading="loading" @click.prevent="createEvent" class="d-flex justify-content-center ma-4"
                        size="x-large" color="blue-darken-4">Create</v-btn>
